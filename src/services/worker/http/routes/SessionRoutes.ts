...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getActiveAgent(): SDKAgent | GeminiAgent | OpenRouterAgent | CustomOpenAIAgent {
    if (isCustomOpenAISelected()) {
      if (isCustomOpenAIAvailable()) {
        ...
        throw new Error('Custom OpenAI provider selected but configuration missing. Set CLAUDE_MEM_CUSTOM_OPENAI_BASE_URL and CLAUDE_MEM_CUSTOM_OPENAI_MODEL in settings.');
      }
    }
   ...
   */
  private getSelectedProvider(): 'claude' | 'gemini' | 'openrouter' | 'custom_openai' {
  ...
  private startGeneratorWithProvider(
    ...
    provider: 'claude' | 'gemini' | 'openrouter' | 'custom_openai',
    source: string
  ): void {
    if (!session) return;
    ...
    const agent = provider === 'custom_openai'
      ? this.customOpenAIAgent
      : (provider === 'openrouter' ? this.openRouterAgent : (provider === 'gemini' ? this.geminiAgent : this.sdkAgent));
    const agentName = provider === 'custom_openai'
      ? 'Custom OpenAI'
